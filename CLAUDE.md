...
pnpm --filter @chonburi/web test        # 222 unit tests (safeUrl XSS prevention, apiBase URL resolution, trafficSim model contract, searchCities geocoding, presets satelliteFreshness + LENSES/ALL_LAYERS/COMPUTED_LAYERS invariants, PARTNER_CITIES data contract, fishery zoneStatus + ZONES contract, tide fmtCountdown + moonEmoji, coastal compass + fmt + aqiColor + aqiBand, time ago relative-format, markets fmtValue + fmtPct + arrow + tickColor, executive execAqiBand + execAqiColor + fmt1 + fmtInt + avgCapacityPct, pmcu hourlyLoad Gaussian model)
